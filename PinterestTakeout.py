...
class TakeoutBoard:
	...
	def export_html(self, board, board_info, board_pins):
		...
		for item in board_pins:
			...
			board_doc.write('<a href="%s" target="_blank">' % item["url"])
			board_doc.write('<p><img height=700 src="%s" id="pin%d">' % (image["url"], i))
			board_doc.write('</a>\n')
			board_doc.write('<p>%d / %d <a href="#pin%d" target="_blank">[#]</a>\n' % (i,len(board_pins), i) )
			board_doc.write('<p>Pin: <a href="%s" target="_blank">%s</a>\n' % (item["url"], item["url"]))
			board_doc.write('<p>Source: <a href="%s" target="_blank">%s</a>\n' % (item["link"],item["link"]))
		board_doc.write('</body>\n')
